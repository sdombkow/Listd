class DealsController < ApplicationController
  ...
  def index
    ...
    # Eager loading pass sets on the user's passes
	  @valid_deals = @user.deals.joins(:deal_set => :fecha).where('fechas.date >= ?', Time.now.to_date).order('fechas.date ASC').paginate(:page => params[:valid_deals_page], :per_page => 5)
	  @past_deals = @user.deals.joins(:deal_set => :fecha).where('fechas.date < ?', Time.now.to_date).order('fechas.date ASC').paginate(:page => params[:past_deals_page], :per_page => 5)
  end
  ...
  def show
    @user = current_user
	  ...
	  if @deal.purchase.stripe_charge_token != nil
	      @customer_card = Stripe::Charge.retrieve(@deal.purchase.stripe_charge_token)
        @card_type = @customer_card.card.type
        @card_four = @customer_card.card.last4
    end
	  logger.error "Purchase #: #{@deal.purchase}"
	  logger.error "Ticket User #: #{@deal.purchase.user}"
	  logger.error "Real User #: #{@user}"
	  if(@user != @deal.purchase.user)
  ...
  def pdfversion
      ...
      send_data pdf.render, :filename => "LISTDDeal(#{@deal.confirmation}).pdf", :type => "application/pdf"
  end
