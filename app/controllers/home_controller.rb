  ...
  def welcome
    ...
    if user_signed_in?
       ...
       # Eager loading pass sets
   	   @passes = @user.passes.includes(:pass_set).where('date >= ? AND redeemed == ?', Time.now.to_date, true).order('updated_at DESC').paginate(:page => params[:page], :per_page => 3)
   	   @reservations = @user.passes.includes(:pass_set).where('date >= ? AND redeemed == ?', Time.now.to_date, false).order('updated_at DESC').paginate(:page => params[:page], :per_page => 3)
   	end
  end
