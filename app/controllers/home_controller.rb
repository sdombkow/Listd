  ...
  def welcome
    ...
    if user_signed_in?
       ...
       # Eager loading pass sets
   	   ...
   	   @reservations = @user.passes.where('redeemed = ?', false).joins(:pass_set).where('pass_sets.selling_passes = ? AND pass_sets.date >= ?', false, Time.now.to_date+1).order('pass_sets.date ASC').limit(3)
   	end
  end
