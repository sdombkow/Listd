class PassesController < ApplicationController
  ...
  def index
      ...
      # Eager loading pass sets on the user's passes
  	  @valid_passes = @user.passes.joins(:pass_set => :fecha).where('fechas.date >= ?', Time.now.to_date).order('fechas.date ASC').paginate(:page => params[:valid_passes_page], :per_page => 5)
  	  @past_passes = @user.passes.joins(:pass_set => :fecha).where('fechas.date < ?', Time.now.to_date).order('fechas.date ASC').paginate(:page => params[:past_passes_page], :per_page => 5)
  end
