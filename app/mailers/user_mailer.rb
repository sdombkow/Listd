class UserMailer < ActionMailer::Base
  ...
  def purchase_confirmation_pass(user,pass)
      ...
      pdf.grid([0,0], [4,3]).bounding_box do
   	      pdf.image "#{Rails.root}/app/assets/images/pass_reserv_bg.png", :width => 240, :position => :center, :vposition => :center
   	      if @pass.pass_set.location != nil
   	          pdf.text_box "<color rgb='888888'>#{@pass.pass_set.location.name}</color>",:inline_format => true, :at => [38,295], :height => 90, :width => 200, :size => 21
   	      elsif @pass.pass_set.event != nil
   	          pdf.text_box "<color rgb='888888'>#{@pass.pass_set.event.name}</color>",:inline_format => true, :at => [38,295], :height => 90, :width => 200, :size => 21
   	      end
  ...
  def purchase_confirmation_deal(user,deal)
      @user=user
      @deal=deal
      pdf = Prawn::Document.new
      pdf.define_grid(:columns => 8, :rows => 8, :gutter => 0)
      ...
      attachments["LISTDPASS(#{@deal.confirmation}).pdf"] = pdf.render
   	
      mail(:to => user.email, :subject => "Purchased Confirmed!")
  end
  
  def purchase_confirmation_reservation(user,reservation)
      @user=user
      ...
      attachments["LISTDRESERVATION(#{@reservation.confirmation}).pdf"] = pdf.render
